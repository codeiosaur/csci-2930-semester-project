...
class DatabaseManager:
    ...
    def leaderboard(self, game, userId, point): #Returns a list containing a list of the highest 10 scores and the users score, a list of the corresponding usernames in order, and an integer for the user's rank
        if point:
            ...
            result = cursor.execute(f"""SELECT HighestPoint FROM GameData WHERE UserID = ? and Game = ?;""", (userId, game,)).fetchone()
            ...
            try:
                cursor.execute(f"""SELECT HighestTime FROM GameData WHERE UserID = ? AND Game = ?;""", (userId, game)).fetchone()
            except:
                score = None
            else:
        ...
        scores.append(score)
        usernames.append(cursor.execute(f"""SELECT Username FROM UserData WHERE UserID = ?;""", (userId,)).fetchone()[0])
